...
if (!empty($arResult['ITEMS'])) {
    foreach ($arResult['ITEMS'] as $sKey => $arItem) {
        ...
        $sPreviewText = !empty($sPreviewText) ? TruncateText(strip_tags($sPreviewText), 200) : false;

        $arResult['ITEMS'][$sKey]['PREVIEW_TEXT'] = $sPreviewText;
    ...
    $arView = [
        ...
        'DELAY_SHOW' => false,
        'DISPLAY_PREVIEW' => ArrayHelper::getValue($arParams, 'DISPLAY_PREVIEW') == 'Y',
        'DISPLAY_PROPERTIES' => ArrayHelper::getValue($arParams, 'DISPLAY_PROPERTIES') == 'Y'
    ];
