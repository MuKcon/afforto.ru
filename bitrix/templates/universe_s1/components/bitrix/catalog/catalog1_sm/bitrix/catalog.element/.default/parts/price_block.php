...
<!--                        <div class="item-quantity text-muted" id="<? $arItemIDs['QUANTITY_LIMIT'] ?>"> 
                    ...
                    <div class="item-current-price-ranges-block-title">
                        ...
                        <span data-entity="price-ranges-ratio-header">
                          ...
                          <? if(count($actualItem['ITEM_QUANTITY_RANGES'])>1): ?>                      
                              <span class="tips_icon" onclick="$('.tips_price').toggleClass('hidden');" style="cursor:pointer;/* background-position: 0 -238px; */height: 16px;width: 16px;background: url(/images/icon.png) no-repeat 0px -238px;display: inline-block;/* position: absolute; *//* overflow: hidden; */top: 0;left: 0;width: 16px;height: 16px;"></span>   
                              <script>
                              $(document).on("click",function(e){
                                  var t=e.target;
                                 // console.log(t,$(t).closest(".tips_price").length);
                                  if(!$(t).closest(".tips_price").length && !$(t).hasClass("tips_icon"))
                                	  $('.tips_price').addClass('hidden')
                              });
                              </script>   
                          <?endif; ?>                
                    </div>
                    
                    <div class="tips_price hidden">                        
                        <div class="price-ranges-block-items tips_body" data-entity="price-ranges-body">
                            <?
                            if ($showRanges)
                            {
    ...
    if(!$currentOffer['CAN_BUY']) $flagDisplay = false;

    if(!empty($arResult['FORM_ORDER'])) $flagDisplay = true;
    ?>
    <div class="col-xs-7 column-buy-button" style="<?= $flagDisplay ? '' : 'display: none;' ?>">
                ...
                <?php if ($bUseProductQuantity) { ?>
                    <?php if (!empty($arResult['OFFERS'])) { ?>
                        <?php foreach($arResult['OFFERS'] as $key => $arOffer) {
                            ...
                            $bQuantityTrace = ArrayHelper::getValue($arOffer, 'CATALOG_QUANTITY_TRACE') == 'Y';

                            if ($bOfferCanBuy) {
                                $arCounterSettings = Json::encode([
                                    'bounds' => [
                                        'minimum' => $arOffer['CATALOG_MEASURE_RATIO'],
                                        'maximum' => $bCanBuyZero || !$bQuantityTrace ? false : $arOffer['CATALOG_QUANTITY']
                                    ],
                    ...
                    <?php } else {
                        ...
                        $bQuantityTrace = ArrayHelper::getValue($arResult, 'CATALOG_QUANTITY_TRACE') == 'Y';

                        $arCounterSettings = Json::encode([
                            'bounds' => [
                                'minimum' => ((int)$arResult['PROPERTIES']['MIN_VAL']['VALUE']!="")?(int)$arResult['PROPERTIES']['MIN_VAL']['VALUE']:$arResult['CATALOG_MEASURE_RATIO'],//$arResult['CATALOG_MEASURE_RATIO'],
                                'maximum' => $bCanBuyZero || !$bQuantityTrace ? false : $arResult['CATALOG_QUANTITY']
                            ],
                        ...
                        <div class="item-buttons block-<?= $currentOfferId ?>">
                            <?php if ($canBuy) { ?>
                                <span class="item-quantity-wrap" data-settings="<?= Html::encode($arCounterSettings) ?>">
                                    ...
                                    <?= Html::input('text', null, ((int)$arResult['PROPERTIES']['MIN_VAL']['VALUE']!="")?(int)$arResult['PROPERTIES']['MIN_VAL']['VALUE']:$arResult['CATALOG_MEASURE_RATIO'], [
                                        'data-type' => 'input',
                                        'class' => 'item-quantity-input',
                                        'min' => ((int)$arResult['PROPERTIES']['MIN_VAL']['VALUE']!="")?(int)$arResult['PROPERTIES']['MIN_VAL']['VALUE']:$arResult['CATALOG_MEASURE_RATIO']
                                    ]) ?>
                                    <?= Html::tag('a', '+', [
                                        'href' => 'javascript:void(0)',
                                        'data-type' => 'button',
