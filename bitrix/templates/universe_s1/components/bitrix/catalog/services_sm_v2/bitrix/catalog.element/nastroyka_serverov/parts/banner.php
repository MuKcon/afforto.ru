...
// Если $sImagePath содержит путь, получить ID файла
$imageId = ArrayHelper::getValue($arResult, 'DETAIL_PICTURE'); // Пример для детальной картинки
if ($imageId) {
    $imageData = CFile::GetFileArray($imageId);
    $alt = !empty($imageData['DESCRIPTION']) ? $imageData['DESCRIPTION'] : $sName; // ALT
    $title = !empty($imageData['TITLE']) ? $imageData['TITLE'] : $sName;         // TITLE
} else {
    $alt = $sName;
    $title = $sName;
}

// Проверяем SEO-настройки детальной картинки
$detailAlt = !empty($arResult['IPROPERTY_VALUES']['ELEMENT_DETAIL_PICTURE_FILE_ALT'])
    ? $arResult['IPROPERTY_VALUES']['ELEMENT_DETAIL_PICTURE_FILE_ALT']
    : $arResult['NAME'];

$detailTitle = !empty($arResult['IPROPERTY_VALUES']['ELEMENT_DETAIL_PICTURE_FILE_TITLE'])
            ...
            <div class="service-header-with-title service-header-image-wrapper">
                <div class="service-header-image">
                    <div class="uni-image" style="height: 100%;">
                        ...
                        <div class="big-image" style="background-image:url(<?= $sImagePath ?>)" title="<?= htmlspecialcharsbx($title) ?>" alt="<?= htmlspecialcharsbx($alt) ?>"></div>
                    </div>
                </div>
                <div class="intec-content">
                    <div class="intec-content-wrapper">
                        <div class="service-header-blocks-wrapper service-banner">
                                    ...
                                    <div class="service-header-information-price">
                                        ...
                                        <?php if ($bButtonShow) { ?>
                                            <div class="service-header-information-order">
												...
												<? else: ?>
                                                    <!-- Старая кнопка Заказать услугу, почему то сломалась и не работает -->
