    ...
    <?php foreach ($products as $id => $product) { ?>
        <tr data-product-id="<?= $id ?>">
            ...
            <td class="column-quantity">
                ...
                <?$db_props = CIBlockElement::GetProperty( 13, $product['ID'],  Array("sort"=>"asc"), Array('CODE' => 'MIN_VAL' ));
											$ar_props = $db_props->Fetch()?>
                
                    <div class="quantity-wrapper intec-no-select"
                         data-settings="<?= Html::encode(Json::encode([
                             'bounds' => array(
                                 'minimum' => ( $ar_props['VALUE'] ? $ar_props['VALUE'] : $product['MEASURE']['RATIO']),//$product['MEASURE']['RATIO'],
