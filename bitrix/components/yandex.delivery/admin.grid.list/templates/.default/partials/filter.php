...
/** @var $component \Yandex\Delivery\Components\AdminGridList */
				...
				switch ($filter['type'])
				{
					...
					case 'list':
						...
						<select name="<?= $filter['id']; ?>" id="<?= $filter['id']; ?>">
							<option value=""><?= Loc::getMessage('YANDEX_DELIVERY_T_ADMIN_GRID_LIST_FILTER_ANY'); ?></option>
							<?
							foreach ($filter['items'] as $optionId => $optionValue)
							{
								?>
								<option value="<?= $optionId; ?>" <?= $optionId == $filter['value'] ? 'selected' : ''; ?>><?= Delivery\Utils::htmlEscape($optionValue); ?></option>
								<?
							}
							?>
						</select>
