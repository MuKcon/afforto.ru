...
{
    protected static $langPrefix = 'YANDER_DELIVERY_GRID_LIST_';

    /** @var \Yandex\Delivery\Component\Base\GridList */
    protected $provider;
    protected $viewList;
    ...
    protected function loadFilter()
    {
        ...
        foreach ($this->arResult['FIELDS'] as $fieldName => $field)
        {
            ...
            $item = [
                ...
                'fieldName' => $fieldName,
                'value' => null,
                'name' => $this->getFirstNotEmpty($field, array(
            ...
            else if ($field['USER_TYPE']['BASE_TYPE'] === 'int' && $field['USER_TYPE']['USER_TYPE_ID'] !== 'boolean')
            {
            	$item['type'] = 'number';
    ...
    {
		if (
			!empty($this->arParams['CONTEXT_MENU'])
			|| $this->arParams['CONTEXT_MENU_EXCEL']
			|| $this->arParams['CONTEXT_MENU_SETTINGS']
		)
		{
			$view = $this->getViewList();
			$view->AddAdminContextMenu($this->arParams['CONTEXT_MENU'], $this->arParams['CONTEXT_MENU_EXCEL'], $this->arParams['CONTEXT_MENU_SETTINGS']);
		}
    }
    ...
    public function getProvider($providerType = null)
    {
        if ($this->provider === null)
        {
            ...
            $className = 'Yandex\Delivery\Component\\' . $providerType . '\GridList';

            if (
                !class_exists($className)
                || !is_subclass_of($className, 'Yandex\Delivery\Component\Base\GridList')
            )
            {
